...
<span title="{{ $post->is_visible == 'Y' ? 'La publicación es visible' : 'La publicación está oculta' }}"
...
@if ($post->is_visible == 'Y')
    <div title="Ocultar publicación" class="show_post" data-state="{{ $post->state }}" data-post_id="{{ $post->id }}"
        id="new_icon_visible_{{ $post->id }}">
        <svg fill="none" stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"
            aria-hidden="true" class="h-6 cursor-pointer">
...
@else
    <div title="Hacer visible publicación" class="show_post" data-state="{{ $post->state }}"
        data-post_id="{{ $post->id }}" id="new_icon_visible_{{ $post->id }}">
